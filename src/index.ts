...
export function registerLens(doc: CambriaState, from: string, to: string, lenses: LensSource) : CambriaState {
  doc.registerLens(from,to,lenses)
  return doc
}

export function applyChanges(doc: CambriaState, changes: CambriaBlock[]) : [CambriaState, Backend.Patch] {
  const patch = doc.applyChanges(changes)
  return [ doc, patch ]
}

export function applyLocalChange(doc: CambriaState, request: Backend.Request) : [CambriaState, Patch] {
  let patch = doc.applyLocalChange(request)
  return [ doc, patch ] 
}

export function getChanges(doc: CambriaState, haveDeps: Hash[]): CambriaBlock[] {
...
export class CambriaState {
  ...
  applyLocalChange(request: Backend.Request) : Backend.Patch {
    ...
    if (instance === undefined) {
      throw new RangeError(`cant apply change - no instance for '${this.schema}'`)
    }
  ...
  schemasEndingIn(schema: string) : string[] {
    return this.schemas.filter(n => n !== "mu").sort((a,b) => a === schema ? 1 : -1)
  }
  ...
  applySchemaChanges(blocks: CambriaBlock[], schemas: string[]) : Backend.Patch {
    ...
    for (let schema of schemas) {
      ...
      const [ newDoc, patch ] = Backend.applyChanges(instance.doc, changeCache[schema])
      // FIXME - startOp / seq
      instance.doc = newDoc
